...
## otherwise)
##
## NOTE: CP6 below ("Add Scene Objects for Pick-and-Place") was inserted after CP5
## with my explicit confirmation on [this date]. Everything from the old CP6 onward
## was renumbered by +1 (old CP6→CP7, old CP7→CP8, old CP8+→CP9+). This is the only
## authorized renumbering to date — any future renumbering needs the same explicit
## confirmation, not inference from this note.
...
- CP6 — Add Scene Objects for Pick-and-Place
  Place 3 small objects in the Isaac Sim scene that the arm can actually pick up and
  place, so CP5's recording pipeline (already built and mechanically verified) starts
  producing real manipulation demonstrations instead of arbitrary joint motion. This
  was inserted here specifically because CP7 (policy training) cannot be meaningful
  without something in the scene to manipulate.
  OPEN DECISIONS (do not assume, decide explicitly at start of this checkpoint):
    - Object shapes/sizes: must be graspable within the gripper's real jaw range
      (URDF limits: -0.174533 to 1.74533 rad — actual graspable width in meters not
      yet derived from that angle) and within the arm's grounded reach envelope
      (dx/dy/dz bounds established in CP4).
    - Object physics: rigid body colliders, mass, friction — needed for real
      pick-and-place behavior, not visual placeholders.
    - Placement method: fixed/scripted positions vs. randomized per episode — affects
      CP5 data diversity and CP7 policy generalization, decide explicitly.
    - Visual distinctiveness: whether objects need distinct color/shape for
      instruction-following (e.g. "pick up the red cube") vs. generic identical
      objects — affects what kind of instructions CP5 labels can meaningfully contain.
  Success test: 3 objects present in the scene with real physics (not decoration),
  each confirmed graspable and placeable via the existing arm teleop, within the
  arm's already-verified reach envelope.

- CP7 — Small Policy Training (VLA-style)
  [formerly CP6] Train a small imitation-learning policy on the collected dataset:
  vision + joint state (+ instruction) -> action.
...
- CP8 — Policy Execution / Evaluation
  [formerly CP7] Run the trained policy in sim in place of teleop for the arm (and/or
  base), evaluate task success qualitatively and against whatever metric CP7 defined.

- CP9+ — Not yet scoped (e.g. real hardware deployment, terrain adaptation, etc.)
  [formerly CP8+] Deliberately left unplanned. Do not infer or propose these until
  CP1–CP8 are done — add them explicitly when we actually get there, based on what
  CP1–CP8 reveal.
...
- CP1–CP4: Complete.
- CP5: Functional — recording pipeline built and verified (3 test episodes recorded,
  real synced camera+joint data confirmed via visualization). NOT yet producing
  meaningful demonstrations, since no objects exist in the scene yet (see CP6).
- CP6: Not started.
- CP7 onward: Not started.
...
- Several checkpoints above (CP6, CP7) contain explicit OPEN DECISIONS that are not yet
  resolved. Do not resolve them by assumption — surface them and ask when that
  checkpoint starts.
